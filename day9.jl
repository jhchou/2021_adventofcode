function day9(file)
    ...
    # create matrix pad_m which m padded with 9's; will be 2 larger in each dimension than m
    ...
    ##### Part 2 #####
    function search_basin(x, y) # recursive function to determine basin size; needs access to global 'track' matrix
    ...
    sizes = Vector{Int}() # vector to store basin sizes as we find them
    for lowpoint in findall(lowpoints)
        ...
        track[.!basin_m] .= '0' # fill in all obstacles (9's)
        (x0, y0) = (lowpoint[1] + 1, lowpoint[2] + 1) # start point of basin; add 1 to indices to index padded basin_m
        push!(sizes, search_basin(x0, y0))
     end
